...
function bfs(node) {
  ...
  //create a queue, start with initial node in the queue
  const queue = [node];
  //while our queue is not empty
  while( queue.length > 0) {
    //dequeu and set it to be current node
    let curr = queue.shift();
    //if left isn't null 
    if( curr.left !== null) {
      //enqueue current's left
      queue.push(curr.left)
    }
    //if right isn't null
    if(curr.right !== null) {
      //enqueue curren'ts right
      queue.push(curr.right)
    }
    //add current's value to our result
...
function dfsPre(node) {
  //create result array
  const result = [];
  //create helper function to traverse to a node
